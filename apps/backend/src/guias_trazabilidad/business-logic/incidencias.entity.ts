...
export class IncidenciaDomainEntity {
    ...
    public static create(props: Omit<Props, | 'idIncidencia' | 'fechaIncidencia'>): Result<IncidenciaDomainEntity> {
        if (!props.idResponsable) {
            return Result.failure('Falta el idResponsable');
        }
        return Result.success(new IncidenciaDomainEntity({
            ...props,
    ...
    public cambiatTipoIncidencia(nuevaIncidencia: TipoIncidenciaVO): IncidenciaDomainEntity {
        return this.update({

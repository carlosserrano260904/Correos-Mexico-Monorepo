...
export class UnidadesService {
      ...
      if (!clavesZonaOrigen.includes(dto.claveCuoDestino)) {
        ...
        if (!clavesZonaDestino.includes(oficinaOrigen.clave_cuo)) {
          ...
          console.log(
            ...
            'Claves zona:',
            oficinaOrigen.clave_unica_zona,
          );
          console.log(
            'Oficina destino:',
            oficinaDestino.clave_cuo,
            'Claves zona:',
            oficinaDestino.clave_unica_zona,
          );
          console.log(
            'Destinos permitidos:',
            destinosPermitidos.map((o) => o.clave_cuo),
          );
  ...
  async getOficinasDestinoValidas(placas: string) {
    ...
    const destinosInversos = await this.oficinaRepo
      ...
      .where(
        `:claveOrigen = ANY(STRING_TO_ARRAY(oficina.clave_unica_zona, ','))`,
        {

...
export class CrearIncidenciaHandler implements ICommandHandler<CrearIncidenciaCommand> {
    ...
    async execute(command: CrearIncidenciaCommand): Promise<any> {
        ...
        const tipoIncidenciaResult = TipoIncidenciaVO.create(command.tipoIncidencia as Incidencias)
        ...
        const numeroRastreoResult = NumeroDeRastreoVO.fromString(command.numeroRastreo)
        ...
        const guiaEncontrada = await this.guiaRepository.findByNumeroRastreo(numeroRastreo) // aqui tengo la duda
        // 3. validar si guia existe
        if (!guiaEncontrada) {
            throw new NotFoundException(`El numero de rastreo ${command.numeroRastreo} no fue encontrado.`)
        }

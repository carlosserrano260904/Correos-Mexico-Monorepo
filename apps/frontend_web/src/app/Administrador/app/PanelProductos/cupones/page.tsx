...
type CouponStatus = "activo" | "borrador" | "caducado";
...
const cuponesIniciales: Cupon[] = [
  ...
  },
  {
    ...
    fechaExpiracion: "16 de Abril, 2025",
    productos: "Ropa",
  },
  {
    id: 3,
...
function getStatusBadgeClasses(status: CouponStatus) {
  if (status === "activo") {
...
export default function CuponesPage() {
  ...
  const [nuevoStatus, setNuevoStatus] = useState<"" | CouponStatus>("");
  ...
  const cuponesFiltrados = useMemo(() => {
    ...
    return data;
  }, [cupones, searchTerm, statusFilter]);

  const abrirDetalle = (cupon: Cupon) => {
    setMostrarCrear(false);
    setCuponSeleccionado(cupon);
  };

  const cerrarDetalle = () => setCuponSeleccionado(null);
  ...
  const handleCrearCupon = () => {
    if (!nuevoCodigo) return; // muy simple, solo que tenga código

    const nuevo: Cupon = {
      id: cupones.length > 0 ? Math.max(...cupones.map((c) => c.id)) + 1 : 1,
      codigo: nuevoCodigo,
      vecesUsado: 0,
      status: (nuevoStatus || "activo") as CouponStatus,
  ...
  return (
    <div className="relative flex flex-col space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Cupones</h1>
        <Button
            ...
            {cuponesFiltrados.map((c) => (
              ...
              >
                ...
                <TableCell className="border-0">
                  <Badge className={getStatusBadgeClasses(c.status)}>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 text-sm space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Código del cupón (EJEMPLO10)"
                  value={nuevoCodigo}
              ...
              <div>
                ...
                <select
                  ...
                  value={nuevoStatus}
                  onChange={(e) =>
                    setNuevoStatus(e.target.value as "" | CouponStatus)
                  }
                >
                  <option value="">Status</option>
                  <option value="activo">Activo</option>
          ...
          <div className="w-full max-w-md h-full bg-white shadow-2xl border-l border-gray-200 flex flex-col">
            ...
            <div className="flex items-start justify-between px-6 py-4 border-b border-gray-200">
              <div>
                ...
                <p className="text-xs text-gray-500">Detalles del cupón</p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 text-sm space-y-4">
              <div className="flex justify-between">
                ...
                <span className="text-gray-900">
                  {cuponSeleccionado.vecesUsado}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500">Status</span>
                <Badge className={getStatusBadgeClasses(cuponSeleccionado.status)}>
                  {getStatusLabel(cuponSeleccionado.status)}
                </Badge>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Fecha de creación</span>
                <span className="text-gray-900">
                  {cuponSeleccionado.fechaCreacion}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Fecha de expiración</span>
                <span className="text-gray-900">
                  {cuponSeleccionado.fechaExpiracion}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Productos</span>
                <span className="text-gray-900">
                  {cuponSeleccionado.productos}
                </span>
              </div>
            </div>
